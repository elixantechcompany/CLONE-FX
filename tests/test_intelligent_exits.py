    ...
    def setUp(self):
        self.config = {
            "circuit_breakers": {
                ...
                "drawdown_pause_pct": 3.0,
                "max_daily_drawdown_percent": 5.0,
                "equity_floor_dollars": 4.50,
            },
            "risk_management": {
                "lot_mode": "auto_compound",
                "risk_per_trade_percent": 1.5,
                "compounding": {
                    "enabled": True,
                    "tiers": [
                        {"min_equity": 0.0, "max_equity": 60.0, "lot_size": 0.01},
                        {"min_equity": 60.0, "max_equity": 120.0, "lot_size": 0.02},
                    ]
                }
            }
        }
        ...
        self.rm.reset_daily_metrics_if_needed(current_equity=100.0)

    def test_drawdown_risk_reduction_and_pause(self):
        """Tests that realized losses progressively activate risk halving and trading pauses."""
        # 0. Normal trading
        can_trade, reason = self.rm.check_circuit_breakers(current_equity=100.0)
        self.assertTrue(can_trade)
        self.assertEqual(self.rm.risk_reduction_multiplier, 1.0)

        # 1. Simulate -2.2% loss ($2.20 loss on $100 baseline)
        self.rm.record_trade_result(zone_id=None, profit=-2.20, magic=1001, exit_reason="SL_HIT")
        can_trade, reason = self.rm.check_circuit_breakers(current_equity=97.80)
        self.assertTrue(can_trade)
        self.assertEqual(self.rm.risk_reduction_multiplier, 0.50, "Risk sizing multiplier should be halved at -2% DD")

        # 2. Simulate further loss reaching -3.2% ($3.20 total loss)
        self.rm.record_trade_result(zone_id=None, profit=-1.00, magic=1001, exit_reason="SL_HIT")
        can_trade, reason = self.rm.check_circuit_breakers(current_equity=96.80)
        self.assertFalse(can_trade, "Trading should pause at -3% DD")
        self.assertIn("Drawdown Cooling Pause", reason)
